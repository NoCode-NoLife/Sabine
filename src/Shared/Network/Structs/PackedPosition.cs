...
namespace Sabine.Shared.Network.Structs
{
	...
	public readonly struct PackedPosition
	{
		...
		public PackedPosition(Position pos, Direction dir)
		{
			this.Position = pos;
			this.Direction = dir;
		...
		/// <param name="b2"></param>
		/// <param name="b3"></param>
		public static void ToBytes(Position pos, Direction dir, out byte b1, out byte b2, out byte b3)
		{
			var x = pos.X;
			var y = pos.Y;
		...
		/// <param name="pos"></param>
		/// <param name="dir"></param>
		public static void FromBytes(byte b1, byte b2, byte b3, out Position pos, out Direction dir)
		{
			var x = (b2 >> 6) | 4 * b1;
			var y = (b3 >> 4) | 16 * (b2 & 0x3F);
			dir = (Direction)(b3 & 0x0F);
