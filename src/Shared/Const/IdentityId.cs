...
{
	/// <summary>
	/// Defines an actors basic look. For players, it defines their job
	/// and for NPCs and monsters, it defines their appearance.
	/// </summary>
	/// <remarks>
	/// This enum started out as a byte, evolved to a short, and
	/// eventually became an int. It was hardcoded inside the client for
	/// the longest time, and as such, the values availabe in any given
	/// version were dependant on the client for most intents and
	/// purposes. The values also changed a bit between the earlier
	/// versions, making it necessary for us to convert our internal
	/// values, which mirror the latest version of this enum, to the
	/// appropriate values for the version we're targeting.
	/// 
	/// To find the actual values sent to the client, you can check the
	/// enum script in the system script files or the debug files in the
	/// user folder after zone server start.
	/// </remarks>
	public enum IdentityId : int
	{
		JT_NOVICE = 0,
		JT_SWORDMAN = 1,
		JT_MAGICIAN = 2,
	...
	/// <summary>
	/// Extensions for the <see cref="IdentityId"/> enum.
	/// </summary>
	public static class IdentityIdExtensions
	{
		/// <summary>
		/// Returns the id expected for the given <see cref="IdentityId"/>
		/// when sent to the client.
		/// </summary>
		/// <param name="identityId"></param>
		/// <returns></returns>
		public static int ToNetwork(this IdentityId identityId)
		{
			if (MEnum<IdentityId>.Shared.TryGetValue(identityId, out var id))
				return id;
			...
			// Fallback for monsters
				...
				fallback = IdentityId.JT_PORING;

			if (MEnum<IdentityId>.Shared.TryGetValue(fallback, out id))
				return id;
