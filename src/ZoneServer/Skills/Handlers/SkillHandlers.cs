...
namespace Sabine.Zone.Skills.Handlers
{
	...
	public class SkillHandlers
	{
		...
		{
			foreach (var type in assembly.GetTypes().Where(static a => typeof(ISkillHandler).IsAssignableFrom(a) && !a.IsInterface))
			{
		...
		public TSkillHandler GetHandler<TSkillHandler>(SkillId skillId) where TSkillHandler : ISkillHandler
		{
			...
			lock (_handlers)
			{
				if (!_handlers.TryGetValue(skillId, out handler))
					return default;
			}
